...
void *rapide_thread(void *arg){
  bloc_t bloc;
  bloc_t blocs[2];
  pile *pil;
  int i, nb_blocs;
  static int nb_thread_of_rapide = 0;
  static pthread_mutex_t mut_pile = PTHREAD_MUTEX_INITIALIZER;
  static pthread_mutex_t mut_cpt = PTHREAD_MUTEX_INITIALIZER;
  static pthread_cond_t cond = PTHREAD_COND_INITIALIZER;
  pil=(pile *)arg;
  while(1){
    pthread_mutex_lock(&mut_pile);
    pthread_mutex_lock(&mut_cpt);
    if(pile_vide(pil) && (nb_thread_of_rapide == 0)){
      pthread_mutex_unlock(&mut_pile);
      pthread_mutex_unlock(&mut_cpt);
      break;
    }
    pthread_mutex_unlock(&mut_pile);
    pthread_mutex_unlock(&mut_cpt);
    ...
    while(pile_vide(pil)){
      ...
      pthread_mutex_unlock(&mut_cpt);
      pthread_cond_wait(&cond, &mut_pile);
    }
    pthread_mutex_lock(&mut_cpt);
    nb_thread_of_rapide++;
    pthread_mutex_unlock(&mut_cpt);
    bloc=depile(pil);
    pthread_mutex_unlock(&mut_pile);
    if((unsigned long)(bloc.fin-bloc.debut)<seuil_bloc_long){
      rapide_seq(bloc);
      nb_blocs=0;
    }else{
      nb_blocs=rapide_decoupebloc(bloc,blocs);
...
void rapide(pos_t taille, unsigned int nb_threads) {
    ...
    pthread_t *pth;
    unsigned int i;
    pth=(pthread_t *) malloc(sizeof(pthread_t)*nb_threads);
    ...
    for(i = 0; i < nb_threads; i++){
      assert(pthread_create(pth+1,NULL,rapide_thread, &pil) == 0);
    }
    for(i = 0; i < nb_threads; i++){
